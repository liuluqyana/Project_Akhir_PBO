 ...
 * [OOP: Polymorphism] Override getNamaTable() untuk menentukan target tabel secara dinamis.
 */
public class DBJadwal extends DBHelper {
    ...
    // Mengambil data seluruh armada pesawat dari tabel referensi
    public List<JadwalModel> getDataPesawat() {
        List<JadwalModel> list = new ArrayList<>();
        String query = "SELECT * FROM pesawat";
        try {
            // Menggunakan properti instans (conn, stmt, rs) warisan dari DBHelper
            stmt = conn.createStatement();
            rs = stmt.executeQuery(query);
            while (rs.next()) {
                // [OOP: Encapsulation] Data dibungkus ke dalam entitas objek lewat method setter
                JadwalModel data = new JadwalModel();
                data.setKodePesawat(rs.getString("kodePesawat"));
                data.setPesawat(rs.getString("pesawat"));
                list.add(data);
            }
            tutupResource(); // Method utilitas dari superclass DBHelper
    ...
    // Mengambil data seluruh master kota dari tabel referensi
    ...
    // Mengambil semua data log jadwal penerbangan dari database
    ...
    // Proses eksekusi query INSERT menggunakan PreparedStatement (Keamanan SQL Injection)
    ...
    // Proses eksekusi query DELETE berdasarkan Primary Key kodeJadwal
    ...
    // Proses eksekusi query UPDATE untuk memperbarui record jadwal penerbangan
    ...
    // Validasi data untuk mengecek redudansi/ketersediaan Primary Key sebelum disimpan
